...
cov_inf_ivd |>
  ...
  # choose only factors relating to parent's perceptions/opinions on vaccination, and the vaccination status of the children themselves
  select(starts_with("perception_"), covid_coverage_age, influenza_coverage_age) |>
  
  # filter out cases without valid vaccination statuses 
  filter(!is.na(covid_coverage_age) & !is.na(influenza_coverage_age)) |>
  
  # make coverage varialbes numerical so correlation can be calculated
  mutate(across(c(covid_coverage_age, influenza_coverage_age), as.numeric)) |>
